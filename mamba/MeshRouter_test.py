...
from pymtl import *
from pclib.rtl import TestSourceValRdy, TestSinkUnorderedValRdy
...
class TestHarness( RTLComponent ):
  ...
  def __init__( s, cls, side, nmessages, payload_nbits, src_msgs, sink_msgs ):
    ...
    s.srcs  = [ TestSourceValRdy( mk_bits(msg_nbits), src_msgs[i] ) for i in xrange(5) ]
    s.sinks = [ TestSinkUnorderedValRdy( mk_bits(msg_nbits), sink_msgs[i] ) for i in xrange(5) ]
  ...
  def line_trace( s ):
    # return s.src.line_trace()+" >>> "+s.router.line_trace()+" >>> "+s.sink.line_trace()
...
def test_4x4():
  ...
  run_test( TestHarness( MeshRouter, 4, 256, 1,
                         src_msgs, sink_msgs ) )

def test_8x8():
  src_msgs, sink_msgs = gen_msgs( 8, 256, 77 )
  run_test( TestHarness( MeshRouter, 8, 256, 77,
                         src_msgs, sink_msgs ) )
def test_16x16():
  src_msgs, sink_msgs = gen_msgs( 16, 256, 77 )
  run_test( TestHarness( MeshRouter, 16, 256, 77,
                         src_msgs, sink_msgs ) )
